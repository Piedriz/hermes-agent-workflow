...
- **Email only** → continue with this skill, use `--services email` during auth
  so the consent screen only asks for Gmail scopes.
...
> **Interpreter note:** the google-api-python-client and google-auth libraries
> live in Hermes's venv, not in system `python`. Invoking with bare `python`
> produces `ModuleNotFoundError: No module named 'google'`. Always use the
> venv path above.
